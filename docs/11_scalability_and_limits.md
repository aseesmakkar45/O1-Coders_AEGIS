...
### Graph Computation
The `GraphEngine` sliding window caps at 200 events. Rebuilding a `nx.DiGraph` from 200 events and computing PageRank, Betweenness, and Closeness Centrality is well within real-time constraints — typically completing in under 50ms on modern hardware.
