...
@app.get("/api/history")
def get_incident_history(limit: int = 50, offset: int = 0, node_id: str = None, anomaly_type: str = None, severity: str = None, date_filter: str = None, time_chunk: str = None, since: str = None):
    """
    Purpose: Retrieve paginated forensic logs.
    Inputs: limit, offset, and optional filter queries (node_id, anomaly_type, severity, date, time, since).
    ...
    """
    rows = reconstructor.history_db.query_history(limit=limit, offset=offset, node_id=node_id, anomaly_type=anomaly_type, severity=severity, date_filter=date_filter, time_chunk=time_chunk, since=since)
